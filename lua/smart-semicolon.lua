...
local function get_word() return vim.fn.expand "<cword>" end

local function get_line() return vim.fn.getline "." end
...
local function word_at_end_line(word, line) return line:sub(-string.len(word)) == word end
...
function undo_smart_semicolon()
	if last_ending == nil then
		...
		if word_len == 3 then word_len = 1 end
