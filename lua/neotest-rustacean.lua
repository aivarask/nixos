...
---@return string | nil
NeotestAdapter.root = function(file_name) return cargo.get_root_dir(file_name) end

---@package
---@param rel_path string Path to directory, relative to root
---@return boolean
NeotestAdapter.filter_dir = function(_, rel_path, _) return rel_path ~= "target" end

---@package
---@param file_path string
---@return boolean
NeotestAdapter.is_test_file = function(file_path) return vim.endswith(file_path, ".rs") end
...
NeotestAdapter.discover_positions = function(file_path)
	...
	for _, runnable in pairs(runnables) do
		...
		if pos then
			...
			if pos.type ~= "dir" then table.insert(positions, pos) end
		end
	end
	...
	for _, namespace in ipairs(namespaces) do
		if namespace.name ~= "" then
			---@type string[]
			local child_keys = vim.tbl_filter(function(name) return vim.startswith(name, namespace.name .. "::") end, vim.tbl_keys(tests_by_name))
	...
	local function sort_positions(to_sort)
		for _, item in ipairs(to_sort) do
			if vim.tbl_islist(item) then sort_positions(item) end
		end
	...
	return require("neotest.types.tree").from_list(sorted_positions, function(x) return x.name end)
end
...
function NeotestAdapter.build_spec(run_args)
	...
	if not vim.tbl_contains(supported_types, pos.type) then return end
	local runnable = pos.runnable
	if not runnable then return end
	local context = {
		file = pos.path,
		pos_id = pos.id,
		type = pos.type,
	...
	if run_args.strategy == "dap" then
		...
		local future = nio.control.future()
		dap.start(runnable.args, false, function(strategy) future.set(strategy) end, function(err) future.set_error(err) end)
...
local function get_file_root(tree)
	for _, node in tree:iter_parents() do
		...
		if data and not vim.tbl_contains({ "test", "namespace" }, data.type) then return node end
	end
	return tree
end
...
function NeotestAdapter.results(spec, strategy_result)
	...
	if not vim.tbl_contains({ "file", "test", "namespace" }, context.type) then return results end
	results[ctx_pos_id] = {
		status = "failed",
		output = strategy_result.output,
	}
...
setmetatable(NeotestAdapter, {
	__call = function() return NeotestAdapter end,
})
